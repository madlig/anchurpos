...
import {
  Loader2, TrendingUp, Package, AlertTriangle, Bell, CheckCheck,
  ShoppingCart, Receipt, Users, ChevronRight,
} from "lucide-react";
...
const QUICK_ACTIONS = [
  { label: "Kasir", href: "/manager/pos", icon: ShoppingCart, bg: "#FEF1F5", color: "#E85D8C" },
  { label: "Pengeluaran", href: "/manager/inventory", icon: Receipt, bg: "#EFF6FF", color: "#3B82F6" },
  { label: "Payroll", href: "/manager/employees", icon: Users, bg: "#F5F3FF", color: "#8B5CF6" },
];
    ...
    <div className="page-enter px-5 pt-6 pb-4 md:px-8 md:pt-8">
      ...
      {data && (
        <div className="grid grid-cols-2 gap-3 mb-5 md:hidden page-enter stagger-1">
          ...
          <div className="rounded-2xl p-4" style={{ background: "#fff", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid #F1F5F9" }} data-testid="profit-card">
      ...
      <div className="mb-5 page-enter stagger-2 max-w-5xl">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2.5">
          {QUICK_ACTIONS.map((action) => {
            ...
            return (
              <Link key={action.href} href={action.href} data-testid={`quick-action-${action.label.toLowerCase()}`} className="tap-target">
                <div className="rounded-2xl p-4 text-center" style={{ background: "#fff", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid #F1F5F9" }}>
                  <div className="h-11 w-11 mx-auto rounded-xl flex items-center justify-center mb-2" style={{ background: action.bg }}>
                    <Icon size={19} style={{ color: action.color }} />
                  </div>
                  <p className="text-xs font-semibold" style={{ color: "#334155" }}>{action.label}</p>
                </div>
              </Link>
            );
          })}
