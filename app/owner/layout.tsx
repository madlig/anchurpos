        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              data-testid={`sidebar-owner-${item.label.toLowerCase()}`}
      ...
      <div className="px-3 py-3" style={{ borderTop: "1px solid #F1F5F9" }}>
        <div className="flex items-center gap-3 px-3 py-2 mb-1.5">
          <div className="h-8 w-8 rounded-xl flex items-center justify-center shrink-0" style={{ background: "linear-gradient(135deg,#E85D8C,#C94A73)" }}>
            <span className="text-white font-bold text-xs">{(user?.displayName ?? "O")[0].toUpperCase()}</span>
          </div>
        ...
        <button
          ...
          style={{ color: "#DC2626" }}
          data-testid="sidebar-logout-owner"
        >
          <LogOut size={14} />
          Keluar
        </button>
