const admin = require('firebase-admin');

admin.initializeApp({
  credential: admin.credential.cert({
    projectId: "anchurpos",
    clientEmail: "firebase-adminsdk-fbsvc@anchurpos.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
  })
});

const db = admin.firestore();

async function checkIngs() {
  const snap = await db.collection("ingredients").get();
  snap.docs.forEach(d => {
    const data = d.data();
    if (data.category === "operasional" || data.category === "kemasan" || d.id.includes("plastik") || d.id.includes("stiker")) {
      console.log(d.id, data.name, data.currentStock, data.category);
    }
  });
}

checkIngs().then(() => process.exit(0)).catch(err => { console.error(err); process.exit(1); });
