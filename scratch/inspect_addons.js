const admin = require('firebase-admin');

admin.initializeApp({
  credential: admin.credential.cert({
    projectId: "anchurpos",
    clientEmail: "firebase-adminsdk-fbsvc@anchurpos.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
  })
});

const db = admin.firestore();

async function inspectAddons() {
  const snap = await db.collection("addOns").get();
  console.log("ADDONS:");
  snap.docs.forEach(doc => {
    console.log(`ID: ${doc.id} | Name: ${doc.data().name} | CurrentStock: ${doc.data().currentStock}`);
  });

  const ingSnap = await db.collection("ingredients").get();
  console.log("\nINGREDIENTS:");
  ingSnap.docs.forEach(doc => {
    const d = doc.data();
    console.log(`ID: ${doc.id} | Name: ${d.name} | Category: ${d.category} | Stock: ${d.currentStock} ${d.baseUnit}`);
  });
}

inspectAddons().then(() => process.exit(0)).catch(err => { console.error(err); process.exit(1); });
