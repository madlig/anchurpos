const admin = require('firebase-admin');

admin.initializeApp({
  credential: admin.credential.cert({
    projectId: "anchurpos",
    clientEmail: "firebase-adminsdk-fbsvc@anchurpos.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
  })
});

const db = admin.firestore();

async function inspectLoyangPool() {
  const snap = await db.collection("productions").where("loyangRemaining", ">", 0).get();
  console.log("PRODUCTIONS WITH REMAINING LOYANGS:");
  snap.docs.forEach(doc => {
    const data = doc.data();
    console.log(`Doc ID: ${doc.id} | Variant: ${data.variantId} | Date: ${data.date?.toDate?.().toISOString() || data.date} | LoyangRemaining: ${data.loyangRemaining} | Batches: ${data.batches}`);
  });
}

inspectLoyangPool().then(() => process.exit(0)).catch(err => { console.error(err); process.exit(1); });
