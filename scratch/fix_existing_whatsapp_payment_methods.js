const admin = require('firebase-admin');

admin.initializeApp({
  credential: admin.credential.cert({
    projectId: "anchurpos",
    clientEmail: "firebase-adminsdk-fbsvc@anchurpos.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
  })
});

const db = admin.firestore();

async function fixExistingWhatsAppOrders() {
  const snap = await db.collection("orders").get();
  let updatedCount = 0;

  for (const doc of snap.docs) {
    const data = doc.data();
    const ch = data.orderChannel || data.channel || "";
    const isWA = ch === "whatsapp" || ch === "wa_form";

    if (isWA && (!data.paymentMethod || data.paymentMethod === "cash")) {
      await doc.ref.update({
        paymentMethod: "transfer"
      });
      console.log(`Updated Order ${doc.id} (#${data.orderNumber}): set paymentMethod = 'transfer'`);
      updatedCount++;
    }
  }

  console.log(`Finished! Total WhatsApp orders updated: ${updatedCount}`);
}

fixExistingWhatsAppOrders().then(() => process.exit(0)).catch(err => { console.error(err); process.exit(1); });
